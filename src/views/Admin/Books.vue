    ...
    <div class="admin-page__heading">
      <div class="admin-page__search-container" style="width: 700px">
        ...
        <transition name="fade-popover">
          <div v-show="filterVisible" ref="filterPopoverRef" class="custom-popover">
            <div class="popover-inner">
              ...
              <el-checkbox v-model="tempIncludeDeleted" size="default" style="margin-top: 15px">
                Bao gồm sách đã xóa
              </el-checkbox>
    ...
    >
      <el-form-item label="Tên sách" prop="title">
        <el-input
          ...
          @input="book.title = book.title.trimStart()"
        ></el-input>
      </el-form-item>
      ...
      <el-form-item label="Mô tả ngắn" prop="short_description">
        ...
        <ckeditor v-model="book.short_description" :config="config" :editor="editor" />
      </el-form-item>
      <el-form-item label="Mô tả đầy đủ" prop="description">
        ...
        <ckeditor v-model="book.description" :config="config" :editor="editor" />
      </el-form-item>
    </el-form>
  </Modal>
...
const formRules = {
  ...
  additional_images: [
    { required: true, message: 'Không được để trống ảnh phụ', trigger: 'change' },
    {
  ...
  author: [{ required: true, message: 'Tác giả không được để trống', trigger: 'blur' }],
  publisher: [{ required: true, message: 'Không để trống tên nhà xuất bản', trigger: 'blur' }],
  category: [{ required: true, message: 'Không để trống tên danh mục', trigger: 'blur' }],
  published_year: [{ required: true, message: 'Không để trống năm xuất bản', trigger: 'blur' }],
  stock_quantity: [
...
const handleFileChange = (file, fileList) => {
  // Create a preview URL for raw files (newly uploaded files)
...
const handleExceed = (files) => {
  ...
  // Create a temp file object with preview URL
  const fileObj = {
    raw: newFile,
    name: newFile.name,
  ...
  // Set a small timeout to ensure the list is cleared first
  setTimeout(() => {
    uploadFileList.value = [fileObj]
    handleFileChange(fileObj, [fileObj])
  }, 10)
}
